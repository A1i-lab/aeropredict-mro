 ...
 * exterior. Run: node scripts/convert_aircraft.mjs /path/to/extracted/source
...
if (!source)
  throw new Error(
    "Pass the extracted third_party/flightgear-a320/source.zip directory",
  );
const binary = [],
  views = [],
  accessors = [],
      ...
      bufferView: view(
        execFileSync(
          ...
          [
            ...
            path.join(source, "Models/Fuselage/res", name),
          ],
...
function category(n, centre) {
  if (n === "ApuFlap") return "Fuselage";
  if (/LEAP/.test(n)) return centre[2] > 0 ? "LeftEngine" : "RightEngine";
  if (/NLG|MLG|Gear|SideStay/.test(n)) return "LandingGear";
  if (/Sensor|Antenna|navglass|Light/.test(n)) return "Sensors";
  if (/Rudder|Elevator|Hstabs/.test(n)) return "Tail";
  if (/Wing|Flap|Slat|Spoiler|Fairing|Aileron|Sharklet/.test(n))
    return centre[2] > 0 ? "LeftWing" : "RightWing";
  return "Fuselage";
}
for (const file of ["A320-216.ac", "CFMLEAP1A.ac"]) {
  const lines = fs
    .readFileSync(path.join(source, "Models/Fuselage/res", file), "utf8")
    .split(/\r?\n/);
  let cursor = 1;
  const acmat = [];
...
function emit(name, tex, crease, verts, surfs, loc, acmat) {
  ...
  for (const m of new Set(triangles.map((t) => t.m))) {
    ...
    for (const t of triangles.filter((t) => t.m === m)) {
      for (const r of t.refs) {
        ...
        if (!keys.has(key)) {
          ...
          pos.push(...verts[r[0]].map((v, i) => v + loc[i]));
          nor.push(...n.toArray());
          uv.push(r[1], 1 - r[2]);
        }
    ...
    material.extras = {
      demonstratorPaint: tex === "Fuse-Main.png" && !/Windsheild/.test(name),
    };
  ...
  const cat = category(name, c);
  nodes[cats[cat]].children.push(nodes.length);
  nodes.push({
    name: name
      .replace("Windsheild", "CockpitWindshield")
...
const doc = {
  asset: {
    ...
    copyright:
      "FlightGear A320-family contributors. GPL-2.0. See third_party/flightgear-a320.",
  },
  scene: 0,
  scenes: [{ nodes: [0] }],
  nodes,
...
bh.write("BIN\0", 4);
fs.writeFileSync(
  "frontend/src/assets/a320neo.glb.gz",
