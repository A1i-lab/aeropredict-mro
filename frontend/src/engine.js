...
export function createEngine(host,onSelect,onReady) {
 ...
 const materials=[];
 function mat(color,metalness=.82,roughness=.32){const m=new THREE.MeshStandardMaterial({color,metalness,roughness,side:THREE.DoubleSide}); materials.push(m);return m;}
 const silver=mat(0xafbdc9),dark=mat(0x344453),edge=mat(0xd9e1e7,.9,.2),hot=mat(0x8c7160,.8,.4),black=mat(0x25303b),blue=mat(0x5b889e);
 ...
 function rotor(g,x,r,n,chord,m){let group=new THREE.Group();group.position.x=x;g.add(group);rotors.push(group);const geo=bladeGeometry(r*.3,r,chord,.7);
  const blades=new THREE.InstancedMesh(geo,m.clone(),n);materials.push(blades.material);const matrix=new THREE.Matrix4();for(let k=0;k<n;k++){matrix.makeRotationX(k/n*Math.PI*2);blades.setMatrixAt(k,matrix);}blades.instanceMatrix.needsUpdate=true;blades.userData.id=g.userData.id;group.add(blades);all.push(blades);
  cylinder(g,r*.32,r*.32,.23,x,dark);ring(g,r*.34,x,edge,.035);return group;
 }
 const fan=sections[0];rotor(fan,-2.3,1.68,26,.56,silver);cylinder(fan,.06,.51,.9,-2.92,edge);ring(fan,1.8,-2.46,edge,.08);ring(fan,1.8,-1.88,edge,.06);
 const sh=cylinder(fan,1.83,1.83,.65,-2.15,silver,true,0,Math.PI*1.16);shells.push(sh);
 const comp=sections[1];
 for(let i=0;i<7;i++){const x=-1.48+i*.27,r=1.12-i*.065;rotor(comp,x,r,32,.2,i%2?silver:edge);ring(comp,r+.035,x,blue,.019);}
 cylinder(comp,.32,.42,1.85,-.62,dark);
 const cs=cylinder(comp,1.22,.77,1.98,-.65,silver,true,0,Math.PI*1.05);shells.push(cs);
 [-1.63,.38].forEach(x=>ring(comp,x<0?1.24:.8,x,edge,.06));
 const combust=sections[2];cylinder(combust,.65,.7,1.05,1.02,hot,true,0,Math.PI*1.25);cylinder(combust,.39,.39,1.12,1.02,dark);
 [.55,.8,1.05,1.3,1.52].forEach(x=>ring(combust,.72,x,hot,.025));
 for(let i=0;i<16;i++){const a=i/16*Math.PI*2;const tube=cylinder(combust,.065,.065,.7,1.0,edge);tube.position.y=Math.cos(a)*.6;tube.position.z=Math.sin(a)*.6;}
 const turbine=sections[3];for(let i=0;i<4;i++){rotor(turbine,1.85+i*.23,.72+i*.075,34,.17,i%2?hot:silver);ring(turbine,.76+i*.075,1.85+i*.23,edge,.028);}
 const ts=cylinder(turbine,.78,1.02,1.02,2.18,dark,true,0,Math.PI*1.03);shells.push(ts);
 const exhaust=sections[4];cylinder(exhaust,.43,.16,.95,3.05,dark);cylinder(exhaust,1.05,.69,.8,3.12,silver,true,0,Math.PI*1.25);ring(exhaust,.7,3.53,edge,.045);
 // Shaft, fasteners and visible structural struts.
 cylinder(comp,.12,.12,4.9,.05,edge);
 sections.forEach((g,i)=>{const x=[-1.88,.35,1.52,2.7,3.53][i],r=[1.8,.8,.75,1.05,.7][i];for(let j=0;j<16;j++){let a=j/16*Math.PI*2;const bolt=mesh(g,new THREE.SphereGeometry(.032,6,5),dark,x);bolt.position.y=r*Math.sin(a);bolt.position.z=r*Math.cos(a);}});
 for(let i=0;i<8;i++){let a=i/8*Math.PI*2;const o=mesh(fan,new THREE.BoxGeometry(.12,.72,.045),dark,-1.86);o.position.y=Math.cos(a)*1.36;o.position.z=Math.sin(a)*1.36;o.rotation.x=a;}
